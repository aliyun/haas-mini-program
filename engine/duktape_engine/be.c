...
static char *resolve_path(const char *parent, const char *module_id)
{
    ...
    if (item_main->valuestring) {
        ...
        if (!amp_stat(resolved_path, &sb) && amp_fs_type(sb.st_mode)) {
            cJSON_Delete(json);
            amp_free(pkg_data);
            return resolved_path;
        }
...
void jsengine_eval_file(const char *filename)
{
    ...
    /* read & run js file */
    amp_stat_t sb;
    if (amp_stat(filename, &sb) || !amp_fs_type(sb.st_mode)) {
