...
static duk_ret_t native_aiot_sign(duk_context *ctx)
{
    ...
    ret = amp_thread_create(&iot_task, aiot_sign, iot_sign_params, &iot_task_params, &iot_task_stack_used);
    if (ret != 0) {
...
static duk_ret_t native_aiot_create_device(duk_context *ctx)
{
    ...
    ret = amp_thread_create(&iot_task, aiot_connect, &iot_connect_module, &iot_task_params, &iot_task_stack_used);
    if (ret != 0) {
        amp_warn(MOD_STR, "iot create task failed");
        goto out;
    }
...
void module_iot_register(void)
{
    ...
    AMP_ADD_FUNCTION("sign",         native_aiot_sign, 2);
    AMP_ADD_FUNCTION("postProps",    native_aiot_postProps, 2);
    AMP_ADD_FUNCTION("onProps",      native_aiot_onProps, 2);
    AMP_ADD_FUNCTION("postEvent",    native_aiot_postEvent, 2);
    AMP_ADD_FUNCTION("onService",    native_aiot_onService, 2);
    AMP_ADD_FUNCTION("onConnect",    native_aiot_onConnect, 2);
    AMP_ADD_FUNCTION("onDisconnect", native_aiot_onDisconnect, 2);
    AMP_ADD_FUNCTION("close",        native_aiot_close, 1);
