...
static int tcp_create_routine(tcp_create_param_t *create_param)
{
    ...
    ret = amp_tcp_establish(create_param->host, create_param->port);
...
static int tcp_send_routine(tcp_send_param_t *send_param)
{
    ...
    ret = amp_tcp_write(sock_id, send_param->msg, send_param->msg_len, 0);
...
static void *tcp_recv_routine(void *arg)
{
    ...
    while(1) {
        p->recv_len = amp_tcp_read(sock_id, p->buf, sizeof(p->buf), MAX_TCP_RECV_TIMEOUT);
...
static duk_ret_t native_tcp_receive(duk_context *ctx)
{
    ...
    ret = amp_thread_create(&tcp_recv_task, tcp_recv_routine, recv_param, &tcp_recv_task_params, &tcp_recv_task_stack_used);
    if (ret != 0) {
