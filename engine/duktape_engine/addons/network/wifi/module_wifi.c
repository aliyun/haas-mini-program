...
static void *wifi_check_ip_task(void *arg)
{
    ...
    while (1) {
        ...
        amp_msleep(WIFI_CHECKIP_INTERVAL_MS);
        count++;
    }
...
static duk_ret_t native_wifi_connect(duk_context *ctx)
{
    ...
    ret = amp_thread_create(&wifi_connect_task, wifi_check_ip_task, (void *)js_cb_ref, &wifi_connect_task_params, &wifi_connect_task_stack_used);
...
void module_wifi_register(void)
{
    ...
    AMP_ADD_FUNCTION("getInfo",    native_wifi_getInfo, 0);
