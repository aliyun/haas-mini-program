...
static duk_ret_t native_mqtt_start(duk_context *ctx)
{
    ...
    err = amp_thread_create(&mqtt_task, mqtt_yield_task, init_params, &mqtt_task_params, &mqtt_task_stack_used);
    if (err)
    {
...
void module_mqtt_register(void)
{
    ...
    AMP_ADD_STRING("version", "0.2.2");
    AMP_ADD_FUNCTION("start",       native_mqtt_start, 2);
    AMP_ADD_FUNCTION("subscribe",   native_mqtt_subscribe, 4);
