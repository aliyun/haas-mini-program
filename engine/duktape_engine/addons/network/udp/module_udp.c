...
static duk_ret_t native_udp_bind(duk_context *ctx)
{
    ...
    ret = amp_udp_socket_bind(sock_id, port);
    if (ret < 0) {
...
static void *udp_recv_routine(void *arg)
{
    ...
    amp_udp_close_without_connect(sock_id);
...
static duk_ret_t native_udp_recvfrom(duk_context *ctx)
{
    ...
    ret = amp_thread_create(&udp_recv_task, udp_recv_routine, recv_param, &udp_recv_task_params, &udp_recv_task_stack_used);
    if (ret != 0) {
