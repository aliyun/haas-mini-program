...
{
    duk_context *ctx;
    static unsigned long pwrkey_down_ts = 0;

    if (state == 0) {
        if (amp_uptime() - pwrkey_down_ts > 3500) {
            state = 2;
        }
    } else if (state == 1) {
        pwrkey_down_ts = amp_uptime();
    }
...
static duk_ret_t native_pm_autosleep_mode_set(duk_context *ctx)
{
    ...
    if (amp_system_autosleep(mode) < 0) {
...
static duk_ret_t native_pm_wakelock_timedlock(duk_context *ctx)
{
    ...
    if (amp_wakelock_timedlock(w, msec) < 0) {
...
static duk_ret_t native_pm_pwrkey_on(duk_context *ctx)
{
    ...
    if (amp_pwrkey_notify_register(__pm_pwrkey_state_cb)) {
...
void module_pm_register(void)
{
    ...
    pm_wakelock = amp_wakelock_create("PM.wakelock");
    if (!pm_wakelock) {
