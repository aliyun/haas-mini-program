...
static void audioplayer_play_result_cb(uvoice_event_t *event, void *data)
{
    duk_context *ctx;
    ...
    if (event->code == UVOICE_CODE_PLAYER_STATE &&
		...
		uvoice_event_unregister(UVOICE_EV_PLAYER, audioplayer_play_result_cb, param);
		amp_free(param);
    }
}

static void audioplayer_listplay_result_cb(uvoice_event_t *event, void *data)
{
    duk_context *ctx;
    audio_player_param_t *param = (audio_player_param_t *)data;
    ...
    if (event->code == UVOICE_CODE_PLAYER_STATE &&
		(event->value == PLAYER_STAT_LIST_PLAY_STOP) &&
		amp_uptime() >= param->timestamp) {
	    ctx = be_get_context();
	    be_push_ref(ctx, param->js_cb_ref);
	    if (duk_pcall(ctx, 0) != DUK_EXEC_SUCCESS) {
		...
		uvoice_event_unregister(UVOICE_EV_PLAYER, audioplayer_listplay_result_cb, param);
		amp_free(param);
    }
}
...
static duk_ret_t native_audioplayer_play(duk_context *ctx)
{
	...
	param = amp_malloc(sizeof(audio_player_param_t));
	if (!param) {
    ...
    if (uvoice_event_register(UVOICE_EV_PLAYER,
		audioplayer_play_result_cb, param)) {
...
void module_audioplayer_register(void)
{
    ...
    if (!audioplayer) {
        audioplayer = amp_malloc(sizeof(audio_player_t));
        if (!audioplayer) {
