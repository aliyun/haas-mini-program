...
char *search_js_app_main_entry(void)
{
     ...
     */
    if ((js_app_fd = amp_fopen(js_app_file_name, "r")) != NULL) {
        amp_fclose(js_app_fd);
    ...
    if ((json_fd = amp_fopen(js_app_file_name, "r")) == NULL) {
        amp_error(MOD_STR, "cannot find the file :%s", js_app_file_name);
        return NULL;
    }

    /* read package config file to json_data buffer */
    amp_fseek(json_fd, 0, HAL_SEEK_END, &curpos);
    if (curpos < 0) {
        file_len = amp_ftell(json_fd);
    } else {
        file_len = curpos;
    }
    ...
    amp_fseek(json_fd, 0, HAL_SEEK_SET, &curpos);
...
void amp_task_main()
{
	...
	amp_msleep(300);
    amp_task_deinit();
    amp_semaphore_post(jse_task_exit_sem);
}
...
void jsengine_main(void)
{
    ...
    jse_task_exit_sem = amp_semaphore_create();
    ...
    amp_thread_create(&jsengine_task, be_jse_task_main_entrance, NULL, &task_params, &jsengine_task_stack_used);
}
