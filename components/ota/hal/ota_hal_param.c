...
int ota_read_parameter(ota_boot_param_t *ota_param)
{
    ...
    int param_part = AMP_PARTITION_PARAMER1;
...
#endif
    ret = amp_hal_flash_erase(AMP_PARTITION_PARAMER1, offset, len);
    if(ret >= 0) {
        ret = amp_hal_flash_write(AMP_PARTITION_PARAMER1, &offset, ota_param, len);
        offset = 0x00;
        if(ret >= 0) {
            ret = amp_hal_flash_read(AMP_PARTITION_PARAMER1, &offset, (unsigned char*)&comp_buf, len);
            if(ret >= 0) {
