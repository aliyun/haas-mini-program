...
static int boot_part               = AMP_PARTITION_OTA_TEMP;
...
{   
    ...
    if(param != NULL) {
        ret = 0;
        amp_hal_logic_partition_t  part_info;
        amp_hal_logic_partition_t *p_part_info;
        ...
        if(ret != 0 || param->len == 0) {
            ...
            if (NULL == ota_cache) {
                ...
                while(len > 0) {
                    ...
                    ret = amp_hal_flash_erase(boot_part, ota_receive_total_len + off, block_size);
                    if(ret < 0) {
                        ret = OTA_INIT_FAIL;
                        if(NULL != ota_cache) {
                            ota_free(ota_cache);
...
OTA_WEAK int ota_hal_write(unsigned int *off, char *in_buf, unsigned int in_buf_len)
{
    ...
    if (ota_cache_len == OTA_FLASH_WRITE_CACHE_SIZE) {
        ret = amp_hal_flash_write(boot_part, off, ota_cache, OTA_FLASH_WRITE_CACHE_SIZE);
        if(ret < 0) {
            goto EXIT;
        }
    ...
    if(ota_receive_total_len == ota_fw_size) {
        if (ota_cache_len != 0) {
            ret = amp_hal_flash_write(boot_part, off, ota_cache, ota_cache_len);
...
OTA_WEAK int ota_hal_rollback(void)
{
    ...
    ret = amp_hal_flash_read(AMP_PARTITION_PARAMER1, &offset, &param, sizeof(ota_boot_param_t));
    if(ret < 0) {
...
void ota_hal_image_crc16(unsigned short *outResult )
{
...
OTA_WEAK int ota_hal_boot(ota_boot_param_t *param)
{
    ...
    amp_hal_logic_partition_t  ota_info;
    amp_hal_logic_partition_t  app_info;
    amp_hal_logic_partition_t *p_ota_info = &ota_info;
...
{
    int ret = 0;
    ret = amp_hal_flash_read(boot_part, off, out_buf, out_buf_len);
    return ret;
}

static char tmp_version[128]; 
int ota_subdev_version_get(char *version);
const char *ota_hal_version(unsigned char dev_type, char *dn)
{
    if(dev_type == 1) {//subdev version
        ...
        if(dn == NULL) {//default module
            ...
            ota_subdev_version_get(tmp_version);
            OTA_LOG_I("js ver = %s\n", tmp_version);
        }
        return tmp_version;
    }
