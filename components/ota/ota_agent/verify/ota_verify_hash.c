...
int ota_check_image_hash(int image_len, unsigned char type, char *hash_value, int len)
{
    int ret                   = 0;
    char *rd_buf              = NULL;
...
int ota_check_image(unsigned int size)
{
    ...
    if (size <= sizeof(ota_image_info_t)) {
        ret = OTA_VERIFY_IMAGE_FAIL;
        goto EXIT;
    }
    off_set = bin_size - sizeof(ota_image_info_t);
    OTA_LOG_I("bin size:%d off:%d", bin_size, off_set);
    ret = ota_read(&off_set, (char *)&image_info, sizeof(ota_image_info_t));
    if (ret < 0) {
        ret = OTA_VERIFY_IMAGE_FAIL;
        goto EXIT;
    }
    ...
    bin_size = image_info.image_size;
    ret = ota_check_image_hash(bin_size, OTA_MD5, image_md5, 16);
    if(ret != 0) {
        goto EXIT;
    }
