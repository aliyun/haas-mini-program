...
static void und_package_wireless_info(struct und_wireless_info_t *info)
{
    /* init the default value, in cast that some platform does not realise amp_get_wireless_info */
    info->rssi = -1;
    info->snr  = 80;
    info->per  = 0;

    if (amp_get_wireless_info(info) < 0) {
        und_err("amp_get_wireless_info not implemented, use default value\n");
    }
