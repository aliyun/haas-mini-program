...
{
    return amp_httpc_get_host_by_name(name);
}
...
int32_t httpc_wrapper_socket_create()
{
    ...
    fd = amp_socket_open(AF_INET, SOCK_STREAM, 0);
    if (fd < 0) {
...
static int32_t recv_func(int32_t socket, uint8_t *data, uint32_t size, uint32_t timeout, httpc_handle_t httpc)
{
    ...
    amp_socket_setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (char *)&recv_to, sizeof(recv_to));

    do {
        ts_left = 0;
        if (ts_end > amp_uptime()) {
            ts_left = ts_end - (uint64_t)amp_uptime();
        }
        ...
        ret = amp_socket_select(socket + 1, &sets, NULL, NULL, (ts_left == 0)? NULL: &tv);

        if (ret > 0) {
            if (FD_ISSET(socket, &sets)) {
                if (http_session == NULL) {
                    ret = amp_socket_recv(socket, data + recv_len, size - recv_len, 0);
                } else {
#if CONFIG_HTTP_SECURE
...
static int32_t send_func(int32_t socket, const uint8_t *data, uint32_t size, int flags, httpc_handle_t httpc)
{
    ...
    ts_end = amp_uptime() + HTTP_WRAPPER_SEND_TIMEOUT;
    sent_len = 0;
    ...
    do {
        ...
        tv.tv_sec = ts_left / 1000;
        tv.tv_usec = (ts_left % 1000) * 1000;
        ret = amp_socket_select(socket + 1, NULL, &sets, NULL, &tv);

        if (ret > 0) {
            if (FD_ISSET(socket, &sets)) {
                if (http_session == NULL) {
                    ret = amp_socket_send(socket, data + sent_len, size - sent_len, 0);
                } else {
#if CONFIG_HTTP_SECURE
...
static int ssl_tx(void *context, const unsigned char *buf, size_t size)
{
    ...
    ret = amp_socket_send(httpc_session->socket, buf, size, 0);
    /* amp_printf("%s, size %d, ret %d", __func__, size, ret); */
    return ret;
}
...
static int ssl_rx(void *context, unsigned char *buf, size_t size)
{
    httpc_t *httpc_session = (httpc_t *)context;
    int ret = 0;
    ret = amp_socket_recv(httpc_session->socket, buf, size, 0);
    /* amp_printf("%s, size %d, ret %d", __func__, size, ret); */
    if (ret < 0) {
...
{
    (void)level;
    amp_printf("%s:%d, %s", file, line, str);
}
                              ...
                              const struct sockaddr *name, socklen_t namelen)
{
    ...
    if (http_session->https.is_inited == false) {
        ...
        if (http_session->https.ssl.ca_cert_c != NULL) {
            ...
            if (ret != 0) {
                amp_printf("%s, mbedtls_x509_crt_parse err -0x%x", __func__, -ret);
                goto exit;
            }
        }
        ...
        if (ret != 0) {
            amp_printf("%s, mbedtls_ssl_config_default err -0x%x", __func__, -ret);
            goto exit;
        }
        ...
        if (ret < 0) {
            amp_printf("%s, mbedtls_ssl_setup err -0x%x", __func__, -ret);
            goto exit;
        }
    ...
    do {
        ...
        if (ret != MBEDTLS_ERR_SSL_WANT_READ &&
            ...
            if (ret < 0) {
                amp_printf("%s, mbedtls_ssl_handleshake err -0x%x", __func__, -ret);
                goto exit;
            }
        }
    } while (ret != 0);
...
{
    return amp_httpc_socket_connect(socket, name, namelen);
}
