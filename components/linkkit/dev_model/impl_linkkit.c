...
#else
    #define IMPL_LINKKIT_MALLOC(size)            amp_malloc(size)
    #define IMPL_LINKKIT_FREE(ptr)               {amp_free((void *)ptr);ptr = NULL;}
#endif
...
static void _linkkit_service_list_mutex_lock(void)
{
    ...
    if (ctx->service_list_mutex) {
        amp_mutex_lock(ctx->service_list_mutex);
    }
}
...
static void _linkkit_service_list_mutex_unlock(void)
{
    iotx_linkkit_ctx_t *ctx = _iotx_linkkit_get_ctx();
    if (ctx->service_list_mutex) {
        amp_mutex_unlock(ctx->service_list_mutex);
    }
}
...
static void _iotx_linkkit_upstream_mutex_lock(void)
{
    ...
    if (ctx->upstream_mutex) {
        amp_mutex_lock(ctx->upstream_mutex);
    }
}
...
static void _iotx_linkkit_upstream_mutex_unlock(void)
{
    iotx_linkkit_ctx_t *ctx = _iotx_linkkit_get_ctx();
    if (ctx->upstream_mutex) {
        amp_mutex_unlock(ctx->upstream_mutex);
    }
}
...
#ifdef DEVICE_MODEL_GATEWAY
    ctx->upstream_mutex = amp_mutex_create();
    if (ctx->upstream_mutex == NULL) {
        amp_mutex_destroy(ctx->mutex);
        ctx->is_opened = 0;
        return STATE_SYS_DEPEND_MUTEX_CREATE;
    }
#endif
...
#if !defined(DEVICE_MODEL_RAWDATA_SOLO)
    ctx->service_list_mutex = amp_mutex_create();
    if (ctx->service_list_mutex == NULL) {
        amp_mutex_destroy(ctx->mutex);
        amp_mutex_destroy(ctx->upstream_mutex);
        ctx->is_opened = 0;
        return STATE_SYS_DEPEND_MUTEX_CREATE;
    }
    ctx->service_list_num = 0;
...
#endif
#if !defined(DEVICE_MODEL_RAWDATA_SOLO)
        amp_mutex_destroy(ctx->service_list_mutex);
#endif /* #if !defined(DEVICE_MODEL_RAWDATA_SOLO) */
        amp_mutex_destroy(ctx->mutex);
        ctx->is_opened = 0;
        return res;
...
static int _iotx_linkkit_slave_connect(int devid)
{
    ...
    if (res > SUCCESS_RETURN) {
        ...
        res = amp_semaphore_wait(semaphore, IOTX_LINKKIT_SYNC_DEFAULT_TIMEOUT_MS);
        if (res < SUCCESS_RETURN) {
            _iotx_linkkit_upstream_mutex_lock();
