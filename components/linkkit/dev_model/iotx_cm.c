...
int iotx_cm_connect(int fd, uint32_t timeout)
{
    ...
    amp_mutex_lock(fd_lock);
    connect_func = _cm_fd[fd]->connect_func;
    amp_mutex_unlock(fd_lock);
    ...
    if (ret == 0) {
        ...
        if (inited_conn_num == 1) {
            ...
            ret = amp_thread_create(&yield_thread, _iotx_cm_yield_thread_func, NULL,
