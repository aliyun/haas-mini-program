...
iotx_cm_connection_t *iotx_cm_open_coap(iotx_cm_init_param_t *params)
{
    ...
    _coap_conncection->list_lock = amp_mutex_create();
...
failed:
    if (_coap_conncection != NULL) {
        if (_coap_conncection->list_lock != NULL) {
            amp_mutex_destroy(_coap_conncection->list_lock);
        }
...
static int iotx_set_devinfo(iotx_coap_device_info_t *p_devinfo)
{
    ...
    amp_snprintf(p_devinfo->device_id, IOTX_PRODUCT_KEY_LEN + IOTX_DEVICE_NAME_LEN + 2, "%s.%s", p_devinfo->product_key,
...
static int  _coap_connect(uint32_t timeout)
{
    ...
    amp_snprintf(url, 100, IOTX_COAP_SERVER_URI, product_key);
    config->p_url = url;
...
static void _coap_response_default(void *p_arg, void *p_message)
{
    ...
    amp_mutex_lock(_coap_conncection->list_lock);
    list_for_each_entry_safe(node, next, &g_coap_response_list, linked_list, coap_response_node_t) {
        if (node->token_num == token) {
            ...
            cm_free(node->topic);
            cm_free(node);
            amp_mutex_unlock(_coap_conncection->list_lock); /* do not lock while callback */
    ...
    amp_mutex_unlock(_coap_conncection->list_lock);
}
