...
#else
    #define IMPL_SOLO_MALLOC(size)            amp_malloc(size)
    #define IMPL_SOLO_FREE(ptr)               {amp_free((void *)ptr);ptr = NULL;}
#endif
...
static void _linkkit_solo_mutex_lock(void)
{
    ...
    if (linkkit_solo_ctx->mutex) {
        amp_mutex_lock(linkkit_solo_ctx->mutex);
    }
}
...
static void _linkkit_solo_mutex_unlock(void)
{
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx = _linkkit_solo_legacy_get_ctx();
    if (linkkit_solo_ctx->mutex) {
        amp_mutex_unlock(linkkit_solo_ctx->mutex);
    }
}
...
static void _linkkit_solo_upstream_mutex_lock(void)
{
    ...
    if (linkkit_solo_ctx->upstream_mutex) {
        amp_mutex_lock(linkkit_solo_ctx->upstream_mutex);
    }
}
...
static void _linkkit_solo_upstream_mutex_unlock(void)
{
    linkkit_solo_legacy_ctx_t *linkkit_solo_ctx = _linkkit_solo_legacy_get_ctx();
    if (linkkit_solo_ctx->upstream_mutex) {
        amp_mutex_unlock(linkkit_solo_ctx->upstream_mutex);
    }
}
                                   ...
                                   linkkit_cloud_domain_type_t domain_type, void *user_context)
{
    ...
    /* Create Mutex */
    linkkit_solo_ctx->mutex = amp_mutex_create();
    if (linkkit_solo_ctx->mutex == NULL) {
        linkkit_solo_ctx->is_started = 0;
        return FAIL_RETURN;
    }

    linkkit_solo_ctx->upstream_mutex = amp_mutex_create();
    if (linkkit_solo_ctx->upstream_mutex == NULL) {
        amp_mutex_destroy(linkkit_solo_ctx->mutex);
        linkkit_solo_ctx->is_started = 0;
        return FAIL_RETURN;
    }
    ...
    if (res != SUCCESS_RETURN) {
        amp_mutex_destroy(linkkit_solo_ctx->mutex);
        amp_mutex_destroy(linkkit_solo_ctx->upstream_mutex);
        linkkit_solo_ctx->is_started = 0;
        return FAIL_RETURN;
    }
