...
#else
    #define cm_malloc(size)            amp_malloc(size)
    #define cm_free(ptr)               {amp_free((void *)ptr);ptr = NULL;}
#endif
...
#else
    #define cm_debug(...)        do{amp_printf(__VA_ARGS__);amp_printf("\r\n");}while(0)
    #define cm_info(...)         do{amp_printf(__VA_ARGS__);amp_printf("\r\n");}while(0)
    #define cm_warning(...)      do{amp_printf(__VA_ARGS__);amp_printf("\r\n");}while(0)
    #define cm_err(...)          do{amp_printf(__VA_ARGS__);amp_printf("\r\n");}while(0)
#endif
