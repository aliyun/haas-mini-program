...
#include "mqtt_api.h"
    ...
    #else
        #define mqtt_api_malloc(size)            amp_malloc(size)
        #define mqtt_api_free(ptr)               {amp_free((void *)ptr);ptr = NULL;}
    #endif
...
static int _iotx_dynamic_register_interal(iotx_http_region_types_t region, iotx_dev_meta_info_t *meta_info)
{
    ...
    res = amp_kv_get(AMP_INTERNAL_DEVICESECRET, device_secret, &buffer_len);
    if (strlen(device_secret) == 0) {
        ...
        amp_kv_set(AMP_INTERNAL_DEVICESECRET, meta_info->device_secret, IOTX_DEVICE_SECRET_LEN, 1);
        amp_kv_get(AMP_INTERNAL_DEVICESECRET, device_secret, &buffer_len);
...
void *IOT_MQTT_Construct_Interal(iotx_mqtt_param_t *pInitParams, iotx_dev_meta_info_t meta_info)
{
    ...
    int buffer_len = IOTX_DEVICE_SECRET_LEN;
    amp_kv_get(AMP_INTERNAL_DEVICESECRET, meta_info_interal.device_secret, &buffer_len);
    ...
    iotx_state_event(ITE_STATE_MQTT_COMM, STATE_SUCCESS, "mqtt connected in %d ms", (int)(amp_uptime() - time_now));
