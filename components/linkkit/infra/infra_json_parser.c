...
#else
#define jparser_debug(...)              do{amp_printf(__VA_ARGS__);amp_printf("\r\n");}while(0)
#endif
...
#else
#define jparser_malloc(size)            amp_malloc(size)
#define jparser_free(ptr)               {amp_free((void *)ptr);ptr = NULL;}
#endif
...
#if WITH_MEM_STATS_PER_MODULE
{
    ...
    amp_snprintf(ret, value_len + 1, "%s", value);
    return ret;
}
