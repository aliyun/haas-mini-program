...
/* SDK run into exception when invoking amp_malloc() */
/* SDK调用的系统适配接口 amp_malloc() 返回异常, 未能成功分配内存 */
#define STATE_SYS_DEPEND_MALLOC                     (STATE_SYS_DEPEND_BASE - 0x0001)
/* SDK run into exception when invoking amp_kv_get() */
/* SDK调用的系统适配接口 amp_kv_get() 返回异常, 未能成功根据Key获取Value */
#define STATE_SYS_DEPEND_KV_GET                     (STATE_SYS_DEPEND_BASE - 0x0002)
/* SDK run into exception when invoking amp_kv_set() */
/* SDK调用的系统适配接口 amp_kv_set() 返回异常, 未能成功根据Key写入Value */
#define STATE_SYS_DEPEND_KV_SET                     (STATE_SYS_DEPEND_BASE - 0x0003)
/* SDK run into exception when invoking amp_kv_del() */
/* SDK调用的系统适配接口 amp_kv_del() 返回异常, 未能成功根据Key删除KV对 */
#define STATE_SYS_DEPEND_KV_DELETE                  (STATE_SYS_DEPEND_BASE - 0x0004)
/* SDK run into exception when invoking amp_mutex_create() */
/* SDK调用的系统适配接口 amp_mutex_create() 返回异常, 未能创建一个互斥锁 */
#define STATE_SYS_DEPEND_MUTEX_CREATE               (STATE_SYS_DEPEND_BASE - 0x0005)
/* SDK run into exception when invoking amp_mutex_lock() */
/* SDK调用的系统适配接口 amp_mutex_lock() 返回异常, 未能成功申请互斥锁 */
#define STATE_SYS_DEPEND_MUTEX_LOCK                 (STATE_SYS_DEPEND_BASE - 0x0006)
/* SDK run into exception when invoking amp_mutex_unlock() */
/* SDK调用的系统适配接口 amp_mutex_unlock() 返回异常, 未能成功释放互斥锁 */
...
/* SDK调用的系统适配接口 amp_tcp_read|Write() 或 HAL_SSL_Read|Write() 返回异常, 底层连接已关闭而未能成功进行网络收发 */
...
/* SDK调用的系统适配接口 amp_tcp_read() 或 HAL_SSL_Read() 未能成功在预期的时间内得到网络回应 */
...
/* SDK调用的系统适配接口 amp_tcp_read() 或 HAL_SSL_Read() 返回异常, 未能成功读取一段内容 */
#define STATE_SYS_DEPEND_NWK_READ_ERROR             (STATE_SYS_DEPEND_BASE - 0x000B)
/* SDK run into exception when invoking amp_semaphore_create() */
/* SDK调用的系统适配接口 amp_semaphore_create() 返回异常, 未能成功申请信号量 */
#define STATE_SYS_DEPEND_SEMAPHORE_CREATE           (STATE_SYS_DEPEND_BASE - 0x000C)
/* SDK run into exception when invoking amp_semaphore_wait() */
/* SDK调用的系统适配接口 amp_semaphore_wait() 返回异常, 未能成功在信号量上睡眠 */
#define STATE_SYS_DEPEND_SEMAPHORE_WAIT             (STATE_SYS_DEPEND_BASE - 0x000D)
/* SDK run into exception when invoking amp_snprintf() */
/* SDK调用的系统适配接口 amp_snprintf() 返回异常, 未能成功拼接格式化字符串 */
