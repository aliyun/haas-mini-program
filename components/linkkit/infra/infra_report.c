...
#else
    #define SYS_REPORT_MALLOC(size) amp_malloc(size)
    #define SYS_REPORT_FREE(ptr)    amp_free(ptr)
#endif
...
int iotx_report_firmware_version(void *pclient)
{
    ...
    ret = amp_get_system_version(version);
    if (ret <= 0) {
