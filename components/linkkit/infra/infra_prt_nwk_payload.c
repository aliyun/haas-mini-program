...
#define JSON_PRINT_NEWSTR       amp_printf("%s", newstr);
#define JSON_PRINT_NEWLINE      \
    do { \
        amp_printf("%s", JSON_NEWLINE); \
        if (mark == '>' || mark == '<' || mark == ':') { \
            amp_printf("%c ", mark); \
        } \
    } while(0)
...
int iotx_facility_json_print(const char *str, int level, ...)
{
    ...
    length = strlen(str);
    amp_printf("%s%s", "\033", _color[level]);
    va_start(ap, level);
    mark = va_arg(ap, int);
    JSON_PRINT_NEWLINE;
    va_end(ap);
    ...
    amp_printf("%s", JSON_NEWLINE JSON_NEWLINE);
    amp_printf("%s", "\033[0m");
    return 0;
}
