...
#else
    #define NET_MALLOC(size) amp_malloc(size)
    #define NET_FREE(ptr)    amp_free(ptr)
#endif
...
{
    return amp_tcp_read(pNetwork->handle, buffer, len, timeout_ms);
}


static int write_tcp(utils_network_pt pNetwork, const char *buffer, uint32_t len, uint32_t timeout_ms)
{
    return amp_tcp_write(pNetwork->handle, buffer, len, timeout_ms);
}
...
static int disconnect_tcp(utils_network_pt pNetwork)
{
    ...
    amp_tcp_destroy(pNetwork->handle);
...
static int connect_tcp(utils_network_pt pNetwork)
{
    ...
    pNetwork->handle = amp_tcp_establish(pNetwork->pHostAddress, pNetwork->port);
    if (pNetwork->handle == (uintptr_t)(-1)) {
        return STATE_SYS_DEPEND_NWK_INVALID_HANDLE;
    }
