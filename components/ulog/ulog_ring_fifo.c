...
int uring_fifo_init()
{
    int rc = -1;
    if (ulog_buffer == NULL) {
        ulog_buffer = amp_malloc(DEFAULT_ASYNC_BUF_SIZE);
        if (ulog_buffer != NULL) {
            ulog_queue = amp_queue_create(ulog_buffer, ULOG_SIZE);
            if (NULL == ulog_queue) {
                amp_free(ulog_buffer);
                ulog_buffer = NULL;
            }
        }
    }
...
{
    return amp_queue_send(ulog_queue, (void*)buf, len, 0);
}
...
int uring_fifo_pop_cb(pop_callback cb, void* cb_arg)
{
    ...
    int rc = amp_queue_recv(ulog_queue, tmp_buf, &rcv_size, 1000);
    if ((0 == rc) && (cb != NULL)) {
