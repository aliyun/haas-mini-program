...
static ssl_hooks_t g_ssl_hooks = {amp_malloc, amp_free};
...
extern int amp_kv_set(const char *key, const void *val, int len, int sync);

extern int amp_kv_get(const char *key, void *val, int *buffer_len);
                                   ...
                                   const unsigned char *buf, size_t len)
{
    ...
    if (session->ticket_len > 0) {
        ...
        session->ticket = amp_malloc(session->ticket_len);
...
#define SEED_MAGIC 0x123
    ...
    ret = amp_kv_get(g_seed_key, &seed_val, &seed_len);
    if (ret) {
        seed_val = SEED_MAGIC;
    }
    seed_val += seed;
    amp_srandom(seed_val);
    seed_val = rand();
    amp_kv_set(g_seed_key, &seed_val, sizeof(seed_val), 1);
#else
    amp_srandom(seed);
#endif
}
...
static int _ssl_random(void *p_rng, unsigned char *output, size_t output_len)
{
    ...
    while (rnglen > 0) {
        *(output + rngoffset) = (unsigned char)amp_random(0);
        rngoffset++;
        rnglen--;
    }
    return 0;
                              ...
                              const char *client_pwd, size_t client_pwd_len)
{
...
#elif (TLS_AUTH_MODE == TLS_AUTH_MODE_PSK)
    {
        ...
        psk_identity_len = strlen(auth_type) + strlen(sign_method) + strlen(product_key) + strlen(device_name) + strlen(
                                       timestamp) + 5;
        psk_identity = amp_malloc(psk_identity_len);
    ...
    if (NULL == saved_session) {
        do {
            ...
            unsigned char *save_buf = amp_malloc(TLS_MAX_SESSION_BUF);
            if (save_buf ==  NULL) {
                amp_debug(MOD_STR, " malloc failed\r\n");
                break;
            }

            saved_session = amp_malloc(sizeof(mbedtls_ssl_session));
    ...
    do {
        ...
        new_session = amp_malloc(sizeof(mbedtls_ssl_session));
        if (NULL == new_session) {
            break;
        }
        ...
        if (ret != 0) {
            ...
            if (ret == 0) {
                char key_buf[KEY_MAX_LEN] = {0};
                amp_snprintf(key_buf,KEY_MAX_LEN -1, KV_SESSION_KEY_FMT, addr);
                ret = amp_kv_set(key_buf, (void *)save_buf, real_session_len, 1);
                if (ret < 0) {
