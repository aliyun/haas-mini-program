...
intptr_t amp_udp_socket_create()
{
    int                     rc = -1;
    long                    socket_id = -1;
...
intptr_t amp_udp_socket_bind(intptr_t p_socket, unsigned short port)
{
 ...
 * @see None.
 */
intptr_t amp_udp_create(_IN_ char *host, _IN_ unsigned short port)
{
    int                     rc = -1;
    long                    socket_id = -1;
    char                    port_ptr[6] = {0};
...
intptr_t amp_udp_create_without_connect(const char *host, unsigned short port)
{
    int                flag      = 1;
    int                ret       = -1;
    int                socket_id = -1;
...
int amp_udp_close_without_connect(_IN_ intptr_t sockfd)
{
    return close((int)sockfd);
}


int amp_udp_recvfrom(intptr_t sockfd, NetworkAddr *p_remote,
...
int amp_udp_sendto(intptr_t sockfd, const NetworkAddr *p_remote,
...
int amp_udp_joinmulticast(intptr_t sockfd, char *p_group)
{
    int err       = -1;
    int socket_id = -1;
...
int amp_udp_write(intptr_t p_socket, const unsigned char *p_data,
...
int amp_udp_read_timeout(intptr_t p_socket, unsigned char *p_data,
