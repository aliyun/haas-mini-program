...
{
    return amp_tcp_establish(host_addr, port);
}

int ota_socket_send(int socket, const char *buf, size_t len)
{
    return amp_tcp_write(socket, buf, len, TCP_SEND_TIMEOUT);
}

int ota_socket_recv(int socket, char *buf, size_t len)
{
    return amp_tcp_read(socket, buf, len, TCP_RECV_TIMEOUT);
}

void ota_socket_close(int socket)
{
    amp_tcp_destroy(socket);
}